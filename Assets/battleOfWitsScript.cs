...
public class battleOfWitsScript : MonoBehaviour {
    ...
    void GeneratePuzzle () {
        ...
        for (int q = 0; q < 2; q++) { //Remove duplicate characters, only include earliest occurance
            usedCharacters = "";
            string k = "";
        ...
        SmolBrainAlgorithm(); //See if there is a solution for both sides
        if (!validPuzzle && generationAttempts < 100) {
            goto TryAgain; //If not, choose a different grid
        } else if (!validPuzzle && generationAttempts == 100) {
    ...
    void SmolBrainAlgorithm () {
        validPuzzle = true;
        int blockedBy = 0;
        for (int z = 0; z < usedCharacters.Length; z++) {
            if (usedCharacters.IndexOf(chosenGrid[z]) == -1 || !validPuzzle) {
                continue;
            }
            if (z < 6) { blockedBy += 1; } else if (usedCharacters.IndexOf(chosenGrid[z-6]) != -1) { blockedBy += 1; }
            if (z % 6 == 0) { blockedBy += 1; } else if (usedCharacters.IndexOf(chosenGrid[z-1]) != -1) { blockedBy += 1; }
            if (z % 6 == 5) { blockedBy += 1; } else if (usedCharacters.IndexOf(chosenGrid[z+1]) != -1) { blockedBy += 1; }
            if (z > 29) { blockedBy += 1; } else if (usedCharacters.IndexOf(chosenGrid[z+6]) != -1) { blockedBy += 1; }
            if (blockedBy == 4) { validPuzzle = false; }
        }
    }
